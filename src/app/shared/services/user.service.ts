...
export class UserService {
  ...
  getRoleUserById(id: string): Promise<string> {
    return this.db.object<string>(`users/${id}/role`).valueChanges().pipe(take(1)).toPromise();
  }
  ...
  async deleteUser(user: User): Promise<any> {
    ...
    if (!isClient) {
      ...
      const favoritesUser = myFavorites.map((favorite) =>
        this.db.object(`favorites/${favorite.id}/users/${user.id}`).remove()
      );
